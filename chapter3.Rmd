...
* failures - number of past class failures (numeric: n if 1<=n<3, else 4)
  - "alcohol consumption and failures would have a positive correlation"
...
sel_alc <- select(alc,one_of(c("G3","goout","absences","failures")))
glimpse(sel_alc)
```
...
<br /> Initializing box plots of the varibles as high use in the x-axis and gender defining the colour:
...
<br /> Drawing the box plots and producing summary statistics for each selected variable based on high use of alcohol and gender:

```{r,fig.dim= c(5,4)}
g1 + geom_boxplot() + ylab("grade") + ggtitle("Student grade by alcohol consumption and sex")
...
```{r,fig.dim= c(5,4)}
g2 + geom_boxplot() + xlab("high use") + ggtitle("Student absences by alcohol consumption and sex")
...
```{r,fig.dim= c(5,4)}
g4 + geom_boxplot() + xlab("high use") + ggtitle("Student failures out by alcohol consumption and sex")
alc %>% group_by(sex, high_use) %>% summarise(count = n(), mean_fail = mean(failures))
```
